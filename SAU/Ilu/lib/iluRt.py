...
# EndILUCopyright
#
# $Id: iluRt.py.3,v 1.1 2020/06/12 06:26:19 cvswky Exp $
#
...
class IluObjSurr(IluObjRemote):
	...
	def IluSetCachedValue(self, value_name, value):
		if not self.__dict__.__contains__('__cached_values__'):
			self.__cached_values__ = {}
		self.__cached_values__[value_name] = value

	def IluGetCachedValue(self, value_name):
		if (self.__dict__.__contains__('__cached_values__') and self.__cached_values__.__contains__(value_name)):
...
class BadPickleValue(Exception):
	def __init__(self, value):
		self.value = value
...
if HasVariantSupport:
		...
		def __init__(self, type_name, *private):
			def GetTypeID(_typename):
				def find_type(typename):
					if TypeAliases.__contains__(typename):
						tuid = TypeAliases[typename][0]
						if RegisteredTypes.__contains__(tuid):
	...
	class Pickle:
		def __init__(self, typecode, *value):
			def PickleValue (value, uid):
				if RegisteredTypes.__contains__(uid):
					funcs = RegisteredTypes[uid]
				else:
					raise UnknownTypeID("unknown ILU type UID:"+uid)
				pickle = None
				call = StartPickle()
				try:
	...
	def InputExtensibleRecord (call, base_type_uid):
		...
		if not base_type_uid in uids:
			raise BadPickleValue("pickle doesn't support required value:"+tmp.bytes())
		else:
			return tmp.value()
...
def CheckSibling (disc, arg):
	...
	if disc_sid != arg_sid:
		raise BadParam(str(arg) + " not sibling of " + str(disc))
...
def FixedPointTypeSize (min_num_v, max_num_v):
	if (not min_num_v) or (not max_num_v):
		return 0
	min_num = int(min_num_v)
	max_num = int(max_num_v)
	if (min_num >= 0 and max_num <= 255):
		...
		return 3
	elif (min_num >= 0 and max_num <= 0xFFFFFFFFFFFFFFFF):
		return 4
	elif (min_num >= -0x8000 and max_num <= 0x7FFF):
		return 5
	elif (min_num >= -0x80000000 and max_num <= 0x7FFFFFFF):
		return 6
	elif (min_num >= -0x8000000000000000 and max_num <= 0x7FFFFFFFFFFFFFFF):
		return 7
	else:
		return 0
