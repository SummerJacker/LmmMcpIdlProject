...
// 混合空地编队（UAV 空中领航 + UGV 地面链式跟随）的持久状态。
// 与 consoleFollowFormationRecord 并列，形状不同（air leader + 高度 + 逐链 distance/angle），
// 避免破坏现有纯地面 follow 编队语义。
struct AirGroundFormationRecord {
    bool ready;
    QString state;               // IDLE / READY / DISPATCHING
    QString airLeaderId;         // 空中领航者，如 AV1
    float airAltitudeM;          // 空中领航高度（米）
    QStringList groundChainIds;  // 地面链式跟随者，如 [GV1, GV2]
    QVector<float> distances;    // 逐链 distance（米）
    QVector<float> anglesRad;    // 逐链 angle（弧度）
    QString message;
    QString errorCode;

    AirGroundFormationRecord()
        : ready(false)
        , airAltitudeM(0.0f)
        , state(QStringLiteral("IDLE")) {}
...
extern AirGroundFormationRecord airGroundFormationRecord;
...
// 混合空地编队链式 Unit_Formation 构造：airLeader 作为索引 0，ground 链依次跟随。
// 与 buildGroundFollowFormation 的区别在于额外携带逐链 angle（弧度），供地面链使用。
...
// Register comma-separated in-process Mock units for tests and benchmark runs.
