...
private slots:
    ...
    void testLogicalUnitIdCompatibility() {
        ...
        QVERIFY(logicalUnitIdsEqual("gv003", "GV3"));
        QVERIFY(!logicalUnitIdsEqual("GV1", "GV2"));
        QVERIFY(!logicalUnitIdsEqual("GV1", "AV1"));
        QVERIFY(!logicalUnitIdsEqual("GV", "GV0"));
    }

    void testPublishedConsoleSbhNormalization() {
        const QByteArray raw(
            "ilusbh:sau/console_sau;IDL%3AConsole%2Frpc%3A1.0;"
            "iiop@tcp_127.0.0.1_5000");
        const QByteArray expected(
            "ilusbh:sau/console_sau;IDL%3AConsole%2Frpc%3A1.0;"
            "iiop@tcp_192.168.3.42_5000");
        QCOMPARE(normalizePublishedConsoleSbh(raw.constData(), "192.168.3.42"), expected);
        QCOMPARE(normalizePublishedConsoleSbh(expected.constData(), "192.168.3.42"), expected);
        QVERIFY(normalizePublishedConsoleSbh(nullptr, "192.168.3.42").isEmpty());
    }

    void testDuplicateIluServerIdConflictDetection() {
        const char* gv1 =
            "ilusbh:rikirobot1/vehicle_1;IDL%3AGround_Unit%2Frpc%3A1.0;"
            "iiop@tcp_192.168.3.103_5005";
        const char* gv2 =
            "ilusbh:rikirobot1/vehicle;IDL%3AGround_Unit%2Frpc%3A1.0;"
            "iiop@tcp_192.168.3.102_5001";
        const QString conflict =
            describeIluServerIdConflict("GV1", gv1, "GV2", gv2);
        QVERIFY(!conflict.isEmpty());
        QVERIFY(conflict.contains(QStringLiteral("rikirobot1")));
        QVERIFY(conflict.contains(QStringLiteral("CORBA_BAD_PARAM")));

        const char* gv2Fixed =
            "ilusbh:rikirobot2/vehicle;IDL%3AGround_Unit%2Frpc%3A1.0;"
            "iiop@tcp_192.168.3.102_5001";
        QVERIFY(describeIluServerIdConflict("GV1", gv1, "GV2", gv2Fixed).isEmpty());
    }
    ...
    void testFollowFormationSnapshot_ReportsRequestedAndEffectiveDistance() {
        ...
        QCOMPARE(json.value(QStringLiteral("state")).toString(), QStringLiteral("READY"));
        const QJsonObject follower = json.value(QStringLiteral("followers"))
            .toArray().first().toObject();
        QCOMPARE(follower.value(QStringLiteral("requested_distance_m")).toDouble(), 0.3);
