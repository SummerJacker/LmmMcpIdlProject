...
ConsoleFollowFormationRecord consoleFollowFormationRecord;
AirGroundFormationRecord airGroundFormationRecord;
char* leaderSBH = NULL;
int currentGUVNum = 0;
int currentAUVNum = 0;
...
QByteArray normalizePublishedConsoleSbh(const char* rawSbh, const char* advertisedHost) {
    ...
    QByteArray result(rawSbh);
    if (advertisedHost == nullptr || *advertisedHost == '\0')
        return result;

    const QByteArray replacement = QByteArray("tcp_") + advertisedHost + "_";
    const QByteArray loopbackPrefixes[] = {
        QByteArray("tcp_127.0.0.1_"),
        QByteArray("tcp_localhost_"),
        QByteArray("tcp_0.0.0.0_")
    };
    for (const QByteArray &prefix : loopbackPrefixes) {
        const int pos = result.indexOf(prefix);
        if (pos >= 0) {
            result.replace(pos, prefix.size(), replacement);
            break;
        }
    }
    return result;
...
static bool parseLogicalUnitId(const char* raw, QByteArray* prefix, int* number) {
    if (raw == nullptr || prefix == nullptr || number == nullptr)
        return false;

    const QByteArray id(raw);
    if (id.size() < 3)
        return false;

    int digitPos = 0;
    while (digitPos < id.size() && !std::isdigit(static_cast<unsigned char>(id[digitPos])))
        digitPos++;
    if (digitPos == 0 || digitPos == id.size())
        return false;
    for (int i = digitPos; i < id.size(); ++i) {
        if (!std::isdigit(static_cast<unsigned char>(id[i])))
            return false;
    }

    bool ok = false;
    const int parsed = id.mid(digitPos).toInt(&ok);
    if (!ok)
        return false;
    *prefix = id.left(digitPos).toUpper();
    *number = parsed;
    return true;
}
...
bool logicalUnitIdsEqual(const char* lhs, const char* rhs) {
    ...
    QByteArray lhsPrefix;
    QByteArray rhsPrefix;
    int lhsNumber = 0;
    int rhsNumber = 0;
    return parseLogicalUnitId(lhs, &lhsPrefix, &lhsNumber) &&
           parseLogicalUnitId(rhs, &rhsPrefix, &rhsNumber) &&
           lhsPrefix == rhsPrefix &&
           lhsNumber == rhsNumber;
}

static bool parseIluSbhIdentity(const char* rawSbh, QByteArray* serverId,
                                QByteArray* endpoint) {
    if (rawSbh == nullptr || serverId == nullptr || endpoint == nullptr)
        return false;

    const QByteArray sbh(rawSbh);
    const QByteArray prefix("ilusbh:");
    const int sidStart = sbh.indexOf(prefix);
    const int slash = sbh.indexOf('/', sidStart + prefix.size());
    const int endpointStart = sbh.lastIndexOf("@tcp_");
    if (sidStart < 0 || slash <= sidStart + prefix.size() || endpointStart < 0)
        return false;

    *serverId = sbh.mid(sidStart + prefix.size(),
                        slash - sidStart - prefix.size());
    *endpoint = sbh.mid(endpointStart + 1);
    return !serverId->isEmpty() && !endpoint->isEmpty();
}
...
QString describeIluServerIdConflict(const char* uidA, const char* sbhA,
    ...
    QByteArray sidA;
    QByteArray sidB;
    QByteArray endpointA;
    QByteArray endpointB;
    if (!parseIluSbhIdentity(sbhA, &sidA, &endpointA) ||
        !parseIluSbhIdentity(sbhB, &sidB, &endpointB) ||
        sidA != sidB || endpointA == endpointB) {
        return QString();
    }

    return QString::fromUtf8(
               "ILU配置冲突：单元%1（%2）与单元%3（%4）复用了相同的Server ID“%5”。"
               "ILU会把它们当成同一台服务器，导致其中一台持续返回CORBA_BAD_PARAM。"
               "请把两台车配置为不同的Server ID（例如rikirobot1、rikirobot2）并重启车辆。")
        .arg(QString::fromUtf8(uidA != nullptr ? uidA : "<unknown>"))
        .arg(QString::fromUtf8(endpointA))
        .arg(QString::fromUtf8(uidB != nullptr ? uidB : "<unknown>"))
        .arg(QString::fromUtf8(endpointB))
        .arg(QString::fromUtf8(sidA));
}

static QStringList currentIluServerIdConflicts(char unitType = '\0') {
    ...
    while (ilu_hash_Next(&he, &uid, &sbh)) {
        if (unitType != '\0' && static_cast<const char*>(uid)[0] != unitType)
            continue;
        units.append({QByteArray(static_cast<const char*>(uid)),
                      QByteArray(static_cast<const char*>(sbh))});
    }

    for (int i = 0; i < units.size(); ++i) {
        for (int j = i + 1; j < units.size(); ++j) {
            const QString conflict = describeIluServerIdConflict(
                units[i].uid.constData(), units[i].sbh.constData(),
                units[j].uid.constData(), units[j].sbh.constData());
            if (!conflict.isEmpty())
...
static void reportIluServerIdConflicts(char unitType = '\0') {
    const QStringList conflicts = currentIluServerIdConflicts(unitType);
    for (const QString &conflict : conflicts) {
        qCritical().noquote() << conflict;
        if (st != nullptr)
            emit st->infoAppended(QString::fromUtf8("严重错误：") + conflict);
    }
}

static QByteArray resolveBoundUnitId(const char* requestedUid) {
    if (requestedUid == nullptr || Units_Hash_Table == NULL)
        return QByteArray(requestedUid != nullptr ? requestedUid : "");

    if (ilu_hash_FindInTable(Units_Hash_Table,
                            reinterpret_cast<ilu_refany>(const_cast<char*>(requestedUid))) != ILU_NIL) {
        return QByteArray(requestedUid);
    }

    ilu_HashEnumerator_s he{};
    ilu_refany boundUid = nullptr;
    ilu_refany boundSbh = nullptr;
    ilu_hash_BeginEnumeration(Units_Hash_Table, &he);
    while (ilu_hash_Next(&he, &boundUid, &boundSbh)) {
        Q_UNUSED(boundSbh);
        if (logicalUnitIdsEqual(requestedUid, static_cast<const char*>(boundUid)))
            return QByteArray(static_cast<const char*>(boundUid));
    }
    return QByteArray(requestedUid);
}
...
// 判断是否为无线网卡（WiFi/WLAN）
// Windows: "WLAN", "Wi-Fi", "无线网络"
// Linux: "wlan", "wlp", "wlx"
static bool isWirelessInterface(const QNetworkInterface& iface) {
    ...
    if (name.contains("wlan") || name.contains("wi-fi") || name.contains("wifi") ||
        name.contains(QString::fromUtf8("\xe6\x97\xa0\xe7\xba\xbf"))) {  // "无线"
        return true;
    }
    // Linux common prefixes (wlan0, wlp3s0, wlx...)
...
void getLocalIP(){
    ...
    for (const QNetworkInterface &iface : interfaces) {
        ...
        for (const QNetworkAddressEntry &entry : iface.addressEntries()) {
            ...
            qDebug() << "[Network] auto selected interface (WiFi preferred):"
    ...
    for (const QNetworkInterface &iface : interfaces) {
        ...
        for (const QNetworkAddressEntry &entry : iface.addressEntries()) {
            ...
            strncpy(host, ip_str.toUtf8().constData(), sizeof(host) - 1);
            host[sizeof(host) - 1] = '\0';
            qDebug() << "[Network] auto selected interface (fallback):"
                     << iface.humanReadableName();
            qDebug() << "[Network] auto selected host:" << host;
            return;
        }
...
void registerConsoleToCosNaming() {
    ...
    CORBA_Environment ev;
    initCosNamingEnv(&ev);
    ilu_CORBA_Object consoleObject =
        ilu_CORBA_Object__CreateFromSBH(reachableConsoleSbh.data(), &ev);
...
# error "当前使用的是 DLL 导入库，请不要定义 ILU_NODLL 或类似宏。"
    ...
    */
    ...
    {
        const QByteArray reachableSbh =
            normalizePublishedConsoleSbh(ILU_C_SBHOfObject(serviceObj), host);
        if (reachableSbh != QByteArray(ILU_C_SBHOfObject(serviceObj))) {
            qDebug() << "[Network] callback SBH normalized to:" << reachableSbh;
        }
    }
...
ilu_boolean getUnitObject(Unit_UnitID uid) {
    ...
    Unit_rpc tmp = NULL;
    char *sbh;
    const QByteArray resolvedUidBytes = resolveBoundUnitId(uid);
    ...
    if (strcmp(uid, resolvedUid) != 0) {
        qDebug() << QString::fromUtf8("单元ID已按绑定表解析: %1 -> %2")
                        .arg(uid).arg(resolvedUid);
    }
    
    // 检查是否是同一个单元，如果是则直接返回
    if (currentUID != NULL && strcmp(currentUID, resolvedUid) == 0) {
        qDebug() << QString::fromUtf8("单元%1已经是当前控制单元").arg(resolvedUid);
        return ilu_TRUE;
    }
    
    qDebug() << QString::fromUtf8("正在切换到单元: %1, SBH: %2").arg(resolvedUid).arg(sbh);
    ...
    if (resolvedUid[0] == 'G') {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << QString::fromUtf8("无法创建地面单元对象: %1, 错误: %2").arg(resolvedUid).arg(ev.returnCode);
            ILU_C_EXCEPTION_FREE(&ev);
            return ilu_FALSE;
        }
    ...
    //空中单元
    else if (resolvedUid[0] == 'A') {
        tmp = (Unit_rpc)ILU_C_SBHToObject(sbh, Air_Unit_rpc__MSType, &ev);
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << QString::fromUtf8("无法创建空中单元对象: %1, 错误: %2").arg(resolvedUid).arg(ev.returnCode);
            ILU_C_EXCEPTION_FREE(&ev);
            return ilu_FALSE;
        }
        ...
        qDebug() << QString::fromUtf8("未知的单元类型: %1").arg(resolvedUid);
        return ilu_FALSE;
    }
    ...
    qDebug() << QString::fromUtf8("成功切换到单元: %1, 对象地址: %2").arg(resolvedUid).arg((quintptr)currentUnitObj, 0, 16);
    return ilu_TRUE;
}
...
bool setToLeader() {
    ...
    QByteArray currentConsoleSBH =
        normalizePublishedConsoleSbh(ILU_C_SBHOfObject(serviceObj), host);
    if (currentConsoleSBH.isEmpty()) {
        qDebug() << "failed to setLeader: Console SBH is empty";
        return false;
    }
    qDebug() << "使用控制台SBH:" << currentConsoleSBH;
    Unit_rpc_setRole(currentUnitObj, Unit_UR_Leader, currentConsoleSBH.data(), &ev);
    if (!ILU_C_SUCCESSFUL(&ev)) {
   ...
   //需要根据currentUnitObj来决定是地面车 还是 无人机 来作为当前的领头人
    ...
    if (!ILU_C_SUCCESSFUL(&ev)) {
        qDebug() << "warning: failed to get optional CosNaming SBH:" << ev.returnCode
                 << "，继续设置Leader";
        ILU_C_EXCEPTION_FREE(&ev);
        tmpSBH = NULL;
    } else if (tmpSBH == NULL || *tmpSBH == '\0') {
        qDebug() << "warning: Leader未提供CosNaming SBH，跳过备份同步";
        tmpSBH = NULL;
    } else {
    ...
    const QByteArray rawConsoleSBH(ILU_C_SBHOfObject(serviceObj));
    if (tmpSBH == NULL) {
        if (backupCosObj != NULL) {
            Console_rpc__Free(&backupCosObj);
            backupCosObj = NULL;
    ...
    } else if (QByteArray(tmpSBH) == rawConsoleSBH ||
               QByteArray(tmpSBH) == currentConsoleSBH) {
        ...
        if(!ILU_C_SUCCESSFUL(&ev)) {
            backupCosObj = NULL;
            qDebug() << "warning: failed to create optional backup Cos Server:"
                     << ev.returnCode << "，继续设置Leader";
            ILU_C_EXCEPTION_FREE(&ev);
        } else {
            qDebug() << "get cos obj";

            // Ping失败只关闭可选的备份同步，不影响Leader主流程。
            ILU_C_PingObject(backupCosObj, &ev);
            if (!ILU_C_SUCCESSFUL(&ev)) {
                qDebug() << "warning: failed to connect to optional backup Cos Server:"
                         << ev.returnCode << "，继续设置Leader";
                if (!ILU_C_SUCCESSFUL(&ev))
                    ILU_C_EXCEPTION_FREE(&ev);
                Console_rpc__Free(&backupCosObj);
    ...
    if (backupCosFile != NULL) {
        ...
        qDebug() << "warning: failed to persist leader information";
    }
    //遍历获取表信息
    ...
    while (ilu_hash_Next(&he, &uid, &sbh)) {
        ...
        // Append按值复制结构体；字符串所有权转移给seq，只释放临时外壳。
        ilu_free(node);
    }
    ...
    if (backupCosObj != NULL) {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << "warning: failed to sync optional CosNaming info:"
                     << ev.returnCode;
            ILU_C_EXCEPTION_FREE(&ev);
        } else {
            ...
            Console_rpc_sendMsg(backupCosObj, &m, &ev);
            if (!ILU_C_SUCCESSFUL(&ev)) {
                qDebug() << "warning: failed to send optional Opened info:"
                         << ev.returnCode;
                if (!ILU_C_SUCCESSFUL(&ev))
                    ILU_C_EXCEPTION_FREE(&ev);
            } else {
...
//遍历setFollowMode - 只设置角色和Leader，不设置模式（模式由setGroupMode设置）
    int followerCount = 0;
    int followerConfigured = 0;
    ilu_hash_BeginEnumeration(Units_Hash_Table, &he);
    while (ilu_hash_Next(&he, &uid, &sbh)) {
        // 处理地面单元
        if (((char*)uid)[0] == 'G' &&
            strcmp(currentUID, (char*)uid) != 0) {
            qDebug() << "set follower:" << (char*)uid;
            followerCount++;
            tmpGrdObj = (Ground_Unit_rpc)ILU_C_SBHToObject((char*)sbh, Ground_Unit_rpc__MSType, &ev);
            if (!ILU_C_SUCCESSFUL(&ev) || tmpGrdObj == NULL) {
                qDebug() << "failed to create follower object for" << (char*)uid
                         << ":" << ev.returnCode;
                if (!ILU_C_SUCCESSFUL(&ev))
                    ILU_C_EXCEPTION_FREE(&ev);
                continue;
            ...
            // 先清除上一次编队遗留的模式，避免仅设置Leader时立即驱动Follower。
            bool configured = true;
            Unit_rpc_setMode(tmpGrdObj, Unit_UM_None, &ev);
            if (!ILU_C_SUCCESSFUL(&ev)) {
                qDebug() << "failed to clear mode for" << (char*)uid << ":" << ev.returnCode;
                ILU_C_EXCEPTION_FREE(&ev);
                configured = false;
            }
            Unit_rpc_setMinorMode(tmpGrdObj, Unit_UMM_None, &ev);
            if (!ILU_C_SUCCESSFUL(&ev)) {
                qDebug() << "failed to clear minor mode for" << (char*)uid << ":" << ev.returnCode;
                ILU_C_EXCEPTION_FREE(&ev);
                configured = false;
            }
        ...
        // 处理空中单元
            ...
            // 先清除上一次编队遗留的模式，避免仅设置Leader时立即驱动Follower。
            bool configured = true;
            Unit_rpc_setMode(tmpAirObj, Unit_UM_None, &ev);
            if (!ILU_C_SUCCESSFUL(&ev)) {
                qDebug() << "failed to clear mode for" << (char*)uid << ":" << ev.returnCode;
                ILU_C_EXCEPTION_FREE(&ev);
                configured = false;
            }
            Unit_rpc_setMinorMode(tmpAirObj, Unit_UMM_None, &ev);
            if (!ILU_C_SUCCESSFUL(&ev)) {
                qDebug() << "failed to clear minor mode for" << (char*)uid << ":" << ev.returnCode;
                ILU_C_EXCEPTION_FREE(&ev);
                configured = false;
    ...
    // __Free只释放sequence内部缓冲区；Create返回的sequence外壳仍需单独释放。
    Unit_UnitInfoSeq__Free(seq);
    ilu_free(seq);
    ilu_free(leaderSBH);
    qDebug() << "设置Leader成功，Follower配置:"
             << followerConfigured << "/" << followerCount;
    return true;
}
                               ...
                               Unit_UnitMode mode)
{
    ...
    if (!ILU_C_SUCCESSFUL(&ev)) {
        ...
        ILU_C_EXCEPTION_FREE(&ev);
        success = false;
    } else if (flag != ilu_TRUE) {
        LOG_ERROR("setGroupMode", QString::fromUtf8("[%1] 主模式设置被单元拒绝").arg(uid));
        success = false;
    } else {
...
bool setGroupMode(Unit_UnitMode mode) {
    ...
    const QStringList serverIdConflicts = currentIluServerIdConflicts();
    if (!serverIdConflicts.isEmpty()) {
        reportIluServerIdConflicts();
        qDebug() << QString::fromUtf8("错误：存在ILU Server ID冲突，无法可靠地同时控制这些单元");
        return false;
    }
    ...
    while (ilu_hash_Next(&he, &uid, &sbh)) {
        ...
        if (ok) {
            ...
            LOG_WARN("setGroupMode", QString::fromUtf8("单元 %1 配置失败，继续处理其他单元").arg((char*)uid));
        }
    ...
    const bool allSucceeded = (totalCount > 0 && totalSuccess == totalCount);
    if (allSucceeded){
        FILE* backupCosFile;
        char backupCosSbhValue[MAX_UNITSBH_LENGTH] = "NULL";
        char leaderSbhValue[MAX_UNITSBH_LENGTH] = "";
        ...
        if (backupCosFile != NULL) {
            const int readCount = fscanf(backupCosFile, "%256s\n%256s\n",
                                         backupCosSbhValue, leaderSbhValue);
            fclose(backupCosFile);
            if (readCount < 2) {
                strcpy(backupCosSbhValue, "NULL");
                strncpy(leaderSbhValue, leaderSBH, MAX_UNITSBH_LENGTH - 1);
                leaderSbhValue[MAX_UNITSBH_LENGTH - 1] = '\0';
            }
        } else {
            strncpy(leaderSbhValue, leaderSBH, MAX_UNITSBH_LENGTH - 1);
            leaderSbhValue[MAX_UNITSBH_LENGTH - 1] = '\0';
        ...
        if (backupCosFile != NULL){
            fprintf(backupCosFile, "%s\n%s\n%s\n%d\n%d",
                    backupCosSbhValue, leaderSbhValue,
                    currentLeaderUID, mode, defaultMinorMode);
            fclose(backupCosFile);
...
{
    return requestedDistance < MIN_SAFE_DISTANCE
        ? MIN_SAFE_DISTANCE
        : requestedDistance;
}
    ...
    QString *errorMessage)
{
    ...
    if (value == NULL) {
        if (errorMessage != NULL)
            *errorMessage = QStringLiteral("failed to allocate formation");
        return NULL;
    }
    ...
    for (int i = 0; i < followerIds.size(); ++i) {
        const QByteArray uidBytes = followerIds.at(i).toUtf8();
        ...
        Unit_FloatSeq_Append(&value->distances, distances.at(i), &err);
        Unit_FloatSeq_Append(&value->angles, 0.0f, &err);
    }
...
void freeUnitFormation(Unit_Formation *value)
{
    if (value == NULL)
        return;
    Unit_Formation__Free(value);
    free(value);
}

Unit_Formation *buildAirGroundFormation(
    const QString &airLeaderId,
    const QStringList &groundFollowerIds,
    const QVector<float> &distances,
    const QVector<float> &anglesRad,
    QString *errorMessage)
{
    // 复用链式 parent 计算（airLeader 作为链头，索引 0）
    QString parentError;
    const QStringList parents = groundFollowParentIds(
        airLeaderId, groundFollowerIds, &parentError);
    if (parents.isEmpty() ||
        groundFollowerIds.size() != distances.size() ||
        groundFollowerIds.size() != anglesRad.size()) {
        if (errorMessage != NULL) {
            *errorMessage = !parentError.isEmpty()
                ? parentError
                : QStringLiteral(
                    "air leader, ground followers, and matching distances/angles are required");
        }
        return NULL;
    }
    ...
    const int count = groundFollowerIds.size() + 1;
    value->robot_ids = *Unit_UnitIDSeq_Create(count, NULL);
    value->leader_ids = *Unit_ShortSeq_Create(count, NULL);
    value->distances = *Unit_FloatSeq_Create(count, NULL);
    value->angles = *Unit_FloatSeq_Create(count, NULL);
    ...
    ilu_Error err;
    const QByteArray leaderBytes = airLeaderId.toUtf8();
    char *leaderCopy = (char *)ilu_malloc(leaderBytes.size() + 1);
    strcpy(leaderCopy, leaderBytes.constData());
    Unit_UnitIDSeq_Append(&value->robot_ids, leaderCopy, &err);
    Unit_ShortSeq_Append(&value->leader_ids, 0, &err);
    ...
    for (int i = 0; i < groundFollowerIds.size(); ++i) {
        const QByteArray uidBytes = groundFollowerIds.at(i).toUtf8();
        char *uidCopy = (char *)ilu_malloc(uidBytes.size() + 1);
        strcpy(uidCopy, uidBytes.constData());
        Unit_UnitIDSeq_Append(&value->robot_ids, uidCopy, &err);
        // 链式：第 i 个 follower 跟随第 i-1 个（i=0 时跟随 airLeader 索引 0）
        Unit_ShortSeq_Append(&value->leader_ids, (CORBA_short)i, &err);
        Unit_FloatSeq_Append(&value->distances, distances.at(i), &err);
        Unit_FloatSeq_Append(&value->angles, anglesRad.at(i), &err);
    }
...
static GroupResult _setGroundFormation(Unit_Formation* formation)
{
    ...
    for (int i = 0; i < n_total; i++) {
        ...
        if (dist_i < MIN_SAFE_DISTANCE && i > 0) {  // Leader(i=0) 的 distance 可以是 0
            ...
            dist_i = effectiveGroundFollowDistance(dist_i);
        }
...
FormationResult setFormationWithResult(Unit_Formation* formation) {
    ...
    const int expectedCount = (int)formation->robot_ids._length;
    const bool pureGroundFormation =
        splitResult.airGroup.robot_ids._length == 0 &&
        splitResult.groundGroup.robot_ids._length == formation->robot_ids._length;
    if (pureGroundFormation &&
        ...
        if (ground.robot_ids._length > 0) {
            consoleFollowFormationRecord.leaderId =
                QString::fromUtf8(ground.robot_ids._buffer[0]);
        }
        for (unsigned long i = 1; i < ground.robot_ids._length; ++i) {
            consoleFollowFormationRecord.followerIds.append(
                QString::fromUtf8(ground.robot_ids._buffer[i]));
            const float requested = ground.distances._buffer[i];
            consoleFollowFormationRecord.requestedDistances.append(requested);
...
bool setDefault() {
    ...
    while (ilu_hash_Next(&he, &uid, &sbh)) {
        ...
        if (unitType == 'G') {
            tmpObj = (Unit_rpc)ILU_C_SBHToObject(
        ...
        if (!ILU_C_SUCCESSFUL(&ev) || tmpObj == NULL) {
            qDebug() << QString::fromUtf8("重置%1失败：无法创建对象，%2")
                            .arg((char*)uid).arg(ev.returnCode);
            if (!ILU_C_SUCCESSFUL(&ev))
                ILU_C_EXCEPTION_FREE(&ev);
            allSucceeded = false;
            continue;
        ...
        // 先停再拆关系，避免旧Follow模式在角色切换期间继续驱动车辆。
        Unit_MoveAction stopAction{};
        stopAction._d = Unit_MA_Stop;
        stopAction._u.stopData.keep = ilu_TRUE;
        Unit_rpc_sendMoveAction(tmpObj, &stopAction, &ev);
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << QString::fromUtf8("%1停止失败: %2")
                            .arg((char*)uid).arg(ev.returnCode);
            ILU_C_EXCEPTION_FREE(&ev);
            allSucceeded = false;
        }
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << QString::fromUtf8("%1清除角色失败: %2")
                            .arg((char*)uid).arg(ev.returnCode);
            ILU_C_EXCEPTION_FREE(&ev);
            allSucceeded = false;
        }

        Unit_rpc_setMode(tmpObj, Unit_UM_None, &ev);
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << QString::fromUtf8("%1清除主模式失败: %2")
                            .arg((char*)uid).arg(ev.returnCode);
            ILU_C_EXCEPTION_FREE(&ev);
            allSucceeded = false;
        }

        Unit_rpc_setMinorMode(tmpObj, Unit_UMM_None, &ev);
        if (!ILU_C_SUCCESSFUL(&ev)) {
            qDebug() << QString::fromUtf8("%1清除次模式失败: %2")
                            .arg((char*)uid).arg(ev.returnCode);
            ILU_C_EXCEPTION_FREE(&ev);
            allSucceeded = false;
        }
...
CORBA_boolean handleBindOperation(Unit_UnitID uid, Unit_UnitSBH sbh, ILU_C_ENVIRONMENT *_status)
{
    ...
    char* existingSbh =
        (char*)ilu_hash_FindInTable(Units_Hash_Table, uid);
    if (existingSbh != ILU_NIL) {
        if (strcmp(existingSbh, sbh) == 0) {
            qDebug() << QString::fromUtf8("[bind] 单元%1重复注册，按幂等成功处理").arg(uid);
            return ilu_TRUE;
        }

        // 同一UID重启后SBH可能变化；替换旧绑定并重写备份，而不是返回AlreadyBind。
        ilu_HashEnumerator_s he{};
        ilu_refany storedUid = NULL;
        ilu_refany storedSbh = NULL;
        ilu_hash_BeginEnumeration(Units_Hash_Table, &he);
        while (ilu_hash_Next(&he, &storedUid, &storedSbh)) {
            if (strcmp((char*)storedUid, uid) == 0) {
                char* removedSbh =
                    (char*)ilu_hash_RemoveFromTable(Units_Hash_Table, storedUid);
                ilu_free(storedUid);
                if (removedSbh != NULL)
                    ilu_free(removedSbh);
                break;
        ...
        qDebug() << QString::fromUtf8("[bind] 单元%1的SBH已更新").arg(uid);
    }

    {
        ...
        strcpy(sbhStr, sbh);
        ilu_hash_AddToTable(Units_Hash_Table, uidStr, sbhStr);
        if(existingSbh == ILU_NIL && uidStr[0] == 'G'){
            currentGUVNum++;
        }
        if(existingSbh == ILU_NIL && uidStr[0] == 'A'){
            currentAUVNum++;
        }
        // 全量重写，避免同一UID留下旧SBH。
        backupCosNamingInfoToLocalFile();
...
CORBA_boolean server_Console_rpc_bind(Console_rpc _handle, Unit_UnitID uid, Unit_UnitSBH sbh, ILU_C_ENVIRONMENT *_status) {
    ...
    if (handleBindOperation(uid, sbh, _status)) {
        // 尚未设置Leader且没有陷阱需要同步时，无需提前创建代理。
        // 这也避免绑定阶段让重复Server ID过早污染ILU连接缓存。
        if (currentGUVLeaderObj == NULL && currentAUVLeaderObj == NULL &&
            currentTrapPointsNum == 0) {
...
CORBA_boolean server_Console_rpc_sendMsg(Console_rpc _handle, Console_Message* m, ILU_C_ENVIRONMENT *_status)
{
    ...
    case Console_OP_ReportTrap:
        ...
        emit st->infoAppended(QString::fromUtf8("成功收到汇报的陷阱点(%1,%2)，半径为%3。").arg(m->_u.tp._u.trapPoint2D.point.x).arg(m->_u.tp._u.trapPoint2D.point.y).arg(m->_u.tp._u.trapPoint2D.radius));
    ...
    case Console_OP_AddTrap:
        ...
        emit st->infoAppended(QString::fromUtf8("成功添加陷阱点(%1,%2)，半径为%3！").arg(m->_u.tp._u.trapPoint2D.point.x).arg(m->_u.tp._u.trapPoint2D.point.y).arg(m->_u.tp._u.trapPoint2D.radius));
        break;
