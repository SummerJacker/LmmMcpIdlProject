    ...
    ui(new Ui::MainWindow)
{
    ...
    // MSVC 2017 会把 Qt 5.12 的静态字符串宏中的中文按系统代码页
    // 编译成乱码；本文件的动态中文统一使用 QString::fromUtf8。
    // 同时避开部分机器上不可用的 Fixedsys，保证输出框能显示完整中文。
    const QFont outputFont(QString::fromLatin1("Microsoft YaHei UI"), 9);
    ...
    connect(ui->pushButton_20, &QPushButton::clicked, this, &MainWindow::addFormationUnit);

    disconnectAll(ui->resetFormationButton_2);
    connect(ui->resetFormationButton_2, &QPushButton::clicked, this, &MainWindow::resetFormation);

    disconnectAll(ui->sendFormationButton_2);
    connect(ui->sendFormationButton_2, &QPushButton::clicked, this, &MainWindow::sendFormation);
...
void MainWindow::on_pushButton_5_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)){
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
            ILU_C_EXCEPTION_FREE(&ev);
        } else {
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送前进指令，线速度为%2！").arg(str.c_str()).arg(action._u.goAheadData.speed));
        }
    }
    else{
...
void MainWindow::on_pushButton_8_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            ...
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送后退指令，线速度为%2！").arg(str.c_str()).arg(action._u.goBackwardData.speed));
        }
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_pushButton_6_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            ...
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送左转指令，角速度为%2！").arg(str.c_str()).arg(action._u.turnLeftData.speed));
        }
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_pushButton_7_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            ...
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送右转指令，角速度为%2！").arg(str.c_str()).arg(action._u.turnRightData.speed));
        }
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_pushButton_9_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev)) {
            ...
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送停止指令！").arg(str.c_str()));
        }
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_pushButton_10_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        if(sendCommandSeq())
        {
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送指令序列！").arg(str.c_str()));
        }
        else
        {
            ui->textBrowser_2->append(QString::fromUtf8("向单元%1发送指令序列失败！").arg(str.c_str()));
        }
    }
    else{
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_PBtnClearAllCache_clicked()
{
    if(!remove(backupFileName))
        ui->textBrowser_2->append(QString::fromUtf8("文件%1已经成功删除！").arg(backupFileName));
    else
        ui->textBrowser_2->append(QString::fromUtf8("文件%1不存在！").arg(backupFileName));

    if (!remove(backupCosNamingServerFileName))
        ui->textBrowser_2->append(QString::fromUtf8("文件%1已经成功删除！").arg(backupCosNamingServerFileName));
    else
        ui->textBrowser_2->append(QString::fromUtf8("文件%1不存在！").arg(backupCosNamingServerFileName));
    if (!remove(backupTrapsFileName))
        ui->textBrowser_2->append(QString::fromUtf8("文件%1已经成功删除！").arg(backupTrapsFileName));
    else
        ui->textBrowser_2->append(QString::fromUtf8("文件%1不存在！").arg(backupTrapsFileName));
}
...
//    ui->textBrowser_2->append(QString::fromStdString(str));
    if(getUnitObject((char*)(str.c_str())))
    {
        if(setToLeader())
        {
            const QString actualLeader =
                QString::fromUtf8(currentLeaderUID != NULL ? currentLeaderUID : str.c_str());
            ui->textBrowser_4->setText(actualLeader);
            ui->textBrowser_2->append(
                QString::fromUtf8("成功设置领航者：%1。可继续选择“跟随模式”控制整组车辆。")
                    .arg(actualLeader));
        }
        else
            ui->textBrowser_2->append(QString::fromUtf8("设置领航者失败！"));
...
void MainWindow::on_PBtnSimulateModel_clicked()
{
    if(setGroupMode(Unit_UM_Imitate))
        ui->textBrowser_2->append(QString::fromUtf8("成功设置模拟模式！"));
    else
        ui->textBrowser_2->append(QString::fromUtf8("设置模拟模式失败！"));
}
//设置跟随模式
void MainWindow::on_PBtnFollowModel_clicked()
{
    if(setGroupMode(Unit_UM_Follow))
        ui->textBrowser_2->append(QString::fromUtf8("成功设置跟随模式！"));
    else
        ui->textBrowser_2->append(QString::fromUtf8("设置跟随模式失败！"));
}
...
void MainWindow::on_pushButton_23_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if(setTaskPoint(x, y))
            ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送目标点(%2,%3)！").arg(str.c_str()).arg(x).arg(y));
        else
            ui->textBrowser_2->append(QString::fromUtf8("向单元%1发送目标点(%2,%3)失败！").arg(str.c_str()).arg(x).arg(y));
...
{
    if(clearTraps())
        ui->textBrowser_2->append(QString::fromUtf8("成功清除所有陷阱！"));
    else
        ui->textBrowser_2->append(QString::fromUtf8("清除陷阱失败！"));
...
void MainWindow::on_pushButton_20_clicked()
{
    ...
    if(pointsCount != 0)
    {
        ...
        ui->textBrowser_2->append(QString::fromUtf8("创建空路径，长度为%1！").arg(pointsCount));
    }else
        ui->textBrowser_2->append(QString::fromUtf8("创建失败，路径长度不能为0！"));
}
...
void MainWindow::on_pushButton_22_clicked()
{
	if(currentPointsCount < pointsCount)
	{
        ...
        currentPointsCount ++;
        ui->textBrowser_2->append(QString::fromUtf8("成功添加目标点（%1，%2）！").arg(pointTmp->_u.p2D.x).arg(pointTmp->_u.p2D.y));
	}else
		ui->textBrowser_2->append(QString::fromUtf8("添加失败，路径已达到最大长度%1！").arg(pointsCount));
}
// 录入队形 按钮
void MainWindow::addFormationUnit()
{
    ilu_Error err;
    ...
    // 单元类型由 comboBox 选择（GV/AV）；首次录入时用当前 Leader 类型保证 Leader 正确
    bool isAirFormation = (ui->comboBox_danyuan->currentText() == QString::fromUtf8("AV"));
    if (!currentUnitCount && currentLeaderUID != NULL) {
        isAirFormation = (currentLeaderUID[0] == 'A');
    }

    // 预分配大小 = 全部绑定单元数（支持混合空地逐条录入）
    const int groupNum = currentAUVNum + currentGUVNum;
    const std::string prefix = isAirFormation ? "AV" : "GV";
    if (groupNum <= 0) {
        ui->textBrowser_2->append(QString::fromUtf8("操作失败：当前无绑定单元。"));
        return;
    }
    ...
    if (!currentUnitCount) {
        ...
        if (currentLeaderUID != NULL && ((isAirFormation && currentLeaderUID[0] == 'A') || (!isAirFormation && currentLeaderUID[0] == 'G'))) {
            ...
            currentUnitCount = 1;
            ui->textBrowser_2->append(QString::fromUtf8("已自动录入 Leader：%1").arg(currentLeaderUID));
        }
    }

    // 如果当前只有 Leader（或没有 Leader），继续录入一个 follower
    if (currentUnitCount >= groupNum) {
        ui->textBrowser_2->append(QString::fromUtf8("队形已满（已录入%1/%2）。").arg(currentUnitCount).arg(groupNum));
        return;
    }
    ...
    if (currentLeaderUID != NULL && uid == std::string(currentLeaderUID)) {
        ui->textBrowser_2->append(QString::fromUtf8("%1 已是 Leader，无需重复录入。" ).arg(QString::fromStdString(uid)));
        return;
    }
    ...
    if (sbh == NULL) {
        ui->textBrowser_2->append(QString::fromUtf8("录入失败：%1 未绑定/未连接（找不到 SBH）。").arg(QString::fromStdString(uid)));
        return;
    }

    // 检查是否已录入（避免重复）
    for (int i = 0; i < (int)formation->robot_ids._length; ++i) {
        const char* existing = *(Unit_UnitIDSeq_Nth(&formation->robot_ids, i));
        if (existing != NULL && strcmp(existing, uid.c_str()) == 0) {
            ui->textBrowser_2->append(QString::fromUtf8("%1 已录入，请勿重复。").arg(QString::fromStdString(uid)));
            return;
        }
    }
    ...
    // 角度：UI 输入为度，转为弧度存储
    Unit_FloatSeq_Append(&formation->angles,
        (float)(ui->doubleSpinBox_jiaodu->value() * 3.14159265358979323846 / 180.0), &err);
    currentUnitCount++;

    ui->textBrowser_2->append(QString::fromUtf8("队形信息 %1 添加成功！（跟随索引=%4）（%2/%3）")
                             .arg(robot_id)
...
void MainWindow::on_pushButton_24_clicked()
{
    ...
    if(setTrap(trap))
        ui->textBrowser_2->append(QString::fromUtf8("成功设置陷阱点(%1,%2)陷阱半径为%3！").arg(trap._u.trapPoint2D.point.x).arg(trap._u.trapPoint2D.point.y).arg(trap._u.trapPoint2D.radius));
    else
        ui->textBrowser_2->append(QString::fromUtf8("设置陷阱点(%1,%2)陷阱半径为%3失败！").arg(trap._u.trapPoint2D.point.x).arg(trap._u.trapPoint2D.point.y).arg(trap._u.trapPoint2D.radius));
}

void MainWindow::on_PBtnTargetTrack_clicked()
{
    if(setGroupMinorMode(Unit_UMM_FW_Object))
        ui->textBrowser_2->append(QString::fromUtf8("设置目标跟踪模式成功！"));
    else
        ui->textBrowser_2->append(QString::fromUtf8("设置目标跟踪模式失败！"));
}

void MainWindow::on_PBtnTrajectoryTrack_clicked()
{
    if(setGroupMinorMode(Unit_UMM_FW_Track))
        ui->textBrowser_2->append(QString::fromUtf8("设置轨迹跟踪模式成功！"));
    else
        ui->textBrowser_2->append(QString::fromUtf8("设置轨迹跟踪失败！"));
}

void MainWindow::on_PBtnCloseAllPrograms_clicked()
{
    shutDownAllUnit();
    ui->textBrowser_2->append(QString::fromUtf8("成功关闭所有单元程序！"));
}
...
void MainWindow::on_PBtnResetAllUnits_clicked()
{
    if (setDefault()) {
        ...
        ui->textBrowser_2->append(QString::fromUtf8("成功停止全部单元并重置空地单元关系！"));
    } else {
        ui->textBrowser_2->append(
            QString::fromUtf8("重置未完全成功：部分单元未响应，请检查日志和单元状态！"));
    }
}
//重置队形 按钮
void MainWindow::resetFormation()
{
    if(currentUnitCount)
    {
        currentUnitCount = 0;
...
void MainWindow::sendFormation()
{
    ...
    if (currentUnitCount < 2) {
        ui->textBrowser_2->append(QString::fromUtf8("至少需要 1 个 Leader + 1 个 Follower 才能发送队形。"));
        return;
    }
    ...
    if (currentLeaderUID != NULL) {
        ...
        if (leaderIdx == -1) {
            ui->textBrowser_2->append(QString::fromUtf8("发送失败：当前 Leader(%1) 不在已录入队形中。请重置队形后重新录入。")
        ...
        if (leaderIdx != 0) {
            ...
            ui->textBrowser_2->append(QString::fromUtf8("已将 Leader(%1) 自动调整为队形第一个。")
                                     .arg(currentLeaderUID));
        }
    }

    // 防止“Leader 类型”和“队形类型”不一致（仅真机校验；Mock 单元放宽，避免误拦空地混合队形）
    const char* uid0 = *(Unit_UnitIDSeq_Nth(&formation->robot_ids, 0));
    if (uid0 != NULL) {
        char* uid0sbh = (char*)ilu_hash_FindInTable(Units_Hash_Table, (ilu_refany)uid0);
        const bool uid0Mock = MockRobotSimulator::isMockSbh(uid0sbh);
        if (!uid0Mock && uid0[0] == 'A' && currentAUVLeaderObj == NULL) {
            ui->textBrowser_2->append(QString::fromUtf8("当前 Leader 不是无人机（AV），无法发送无人机队形。请先设置 AV Leader。"));
            return;
        }
        if (!uid0Mock && uid0[0] == 'G' && currentGUVLeaderObj == NULL) {
            ui->textBrowser_2->append(QString::fromUtf8("当前 Leader 不是无人车（GV），无法发送无人车队形。请先设置 GV Leader。"));
            return;
        }
    }
    ...
    if (!result.crossTypeWarnings.empty()) {
        ...
        for (const auto& edge : result.crossTypeWarnings) {
            ui->textBrowser_2->append(QString::fromUtf8("  - [%1 → %2] 已忽略")
    ...
    if (result.isSuccess()) {
        ...
        ui->textBrowser_2->append(QString::fromUtf8("✓ 队形下发成功！（%1个单元，耗时%2ms）")
        ...
        ui->textBrowser_2->append(QString::fromUtf8("⚠ 队形部分成功：成功%1个，失败%2个（耗时%3ms）")
        ...
        for (const auto& ur : failedUnits) {
            QString orphanTag = ur.isOrphan ? QString::fromUtf8("（独立节点）") : QString::fromUtf8("");
            ui->textBrowser_2->append(QString::fromUtf8("  - %1 失败%2: %3")
        ...
        // 全部失败
        int totalFail = result.airResult.failCount + result.groundResult.failCount;
        ui->textBrowser_2->append(QString::fromUtf8("✗ 队形下发失败！（%1个单元失败，耗时%2ms）")
                                 .arg(totalFail).arg(result.elapsedMs));
        
        // 显示失败单元详情
        std::vector<UnitResult> failedUnits = result.getAllFailedUnits();
        for (const auto& ur : failedUnits) {
            ui->textBrowser_2->append(QString::fromUtf8("  - %1: %2")
                                     .arg(ur.uid).arg(ur.errorMsg));
        }
        
        ui->textBrowser_2->append(QString::fromUtf8("请查看控制台日志/单元状态，可重试发送。"));
    }
}
//发送路径按钮
    ...
    if(currentPointsCount == pointsCount)
    {
        ...
        if(getUnitObject((char*)(str.c_str())))
        {
            ...
            if(Ground_Unit_rpc_setTaskPath(currentUnitObj, path2D, &en)&&ILU_C_SUCCESSFUL(&en))
            {
                ...
                Console_PointSeq__Free(path);
                path = NULL;
                ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送路径！").arg(str.c_str()));
            }
            else if(!ILU_C_SUCCESSFUL(&en))
                ui->textBrowser_2->append(QString::fromUtf8("向单元%1发送路径失败，出现异常%2！").arg(str.c_str()).arg(en.returnCode));
            else
                ui->textBrowser_2->append(QString::fromUtf8("向单元%1发送路径失败！").arg(str.c_str()));
        }else
            ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
    }else
        ui->textBrowser_2->append(QString::fromUtf8("路径尚未录入完毕，还需录入%1个点！").arg(pointsCount-currentPointsCount));
}

void MainWindow::showPhotoWindow()
{
//    hideallframe();
//    ui->photoframe->show();
...
void MainWindow::on_pushButton_28_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送拍照指令，频率为%2！").arg(str.c_str()).arg(frequency));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_29_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送停止拍照指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_43_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送解锁指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_31_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送起飞指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_39_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送下降指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.goDownData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_30_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送降落指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_32_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送确认降落指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_44_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送上锁指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_33_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送上升指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.goUpData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_34_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送左旋指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.turnLeftData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_41_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送右旋指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.turnRightData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_35_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送前进指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.goAheadData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_40_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送后退指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.goBackwardData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_37_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送左飞指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.goLeftData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_38_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送右飞指令！,速度为%2 m/s").arg(str.c_str()).arg(action._u.goRightData.speed));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_45_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送控制%2指令！").arg(str.c_str()).arg(strG.c_str()));
        }
        else
                ui->textBrowser_2->append(QString::fromUtf8("操作失败，地面单元%1不存在！").arg(strG.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_47_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送获得权限指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_48_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送释放权限指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!flag) {
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));}
               ...
               ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送控制指令！").arg(str.c_str()));
...
void MainWindow::on_pushButton_50_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送强制降落指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
//
void MainWindow::addAirFollower()
{
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向leader发送控制%1指令！").arg(strA.c_str()));
        }
        else
                ui->textBrowser_2->append(QString::fromUtf8("操作失败，空中单元%1不存在！").arg(strA.c_str()));

}

void MainWindow::removeAirFollower()
{
        ...
        ui->textBrowser_2->append(QString::fromUtf8("成功向leader发送控制%1指令！").arg(strA.c_str()));
                ...
                ui->textBrowser_2->append(QString::fromUtf8("操作失败，地面单元%1不存在！").arg(strA.c_str()));
...
void MainWindow::on_pushButton_53_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
        {ui->textBrowser_2->append(QString::fromUtf8("指令设置角色发送失败，出现异常：%1").arg(ev.returnCode));
            return;}
        Air_Unit_ActionMode actionMode;
        actionMode = Air_Unit_Cruise;
        Air_Unit_rpc_setActionMode (currentUnitObj,actionMode,&ev);
        if (!ILU_C_SUCCESSFUL(&ev))
        {ui->textBrowser_2->append(QString::fromUtf8("指令设置模式发送失败，出现异常：%1").arg(ev.returnCode));
            return;}
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送设置模式为巡航模式指令！").arg(str.c_str()));
        Air_Unit_Point3D po;
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
        {ui->textBrowser_2->append(QString::fromUtf8("指令设置巡航范围发送失败，出现异常：%1").arg(ev.returnCode));
            return;}
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_pushButton_79_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        {ui->textBrowser_2->append(QString::fromUtf8("指令设置模式发送失败，出现异常：%1").arg(ev.returnCode));
            return;}
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送确认抛投指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_77_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送确认火情指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_83_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送取消火情指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_78_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送确认位置指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_85_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送重新计算位置指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_84_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送取消抛投指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_76_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        {ui->textBrowser_2->append(QString::fromUtf8("指令设置模式发送失败，出现异常：%1").arg(ev.returnCode));
            return;}
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送设置模式为空模式指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_80_clicked()
{
    ...
    if(!currentAUVNum)
    {
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，当前绑定单元数为0！"));
        return;
    }
    //当前控制单元数为空，新建队列
    ...
    if(currentControlledCount<currentAUVNum)
    {
        ...
        ui->textBrowser_2->append(QString::fromUtf8("可控无人机%1添加成功！").arg(strA.c_str()));
    }else
        ui->textBrowser_2->append(QString::fromUtf8("数量已满！"));
}
//无人机发送 按钮
void MainWindow::on_pushButton_81_clicked()
{
    if (unitSBHSeq==NULL){
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，请先添加可控制单元！"));
        return;
    }
    string str = "AV";
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
        {ui->textBrowser_2->append(QString::fromUtf8("指令发送可控无人机发送失败，出现异常：%1").arg(ev.returnCode));
            return;
        }
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送发送可控无人机指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
...
void MainWindow::on_pushButton_82_clicked()
{
    if (currentControlledCount!=0){
        currentControlledCount=0;
        Unit_UnitSBHSeq__Free(unitSBHSeq);
        ui->textBrowser_2->append(QString::fromUtf8("已重置添加无人机！"));
    }
    else {
        ui->textBrowser_2->append(QString::fromUtf8("未录入无人机！"));
    }
}
...
void MainWindow::on_pushButton_54_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送设置巡航速度指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_57_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送打开视频流指令，频率为%2！").arg(str.c_str()).arg(frequency));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_55_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送关闭视频流指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
...
void MainWindow::on_pushButton_56_clicked()
{
    ...
    if(getUnitObject((char*)(str.c_str())))
    {
        ...
        if (!ILU_C_SUCCESSFUL(&ev))
            ui->textBrowser_2->append(QString::fromUtf8("指令发送失败，出现异常：%1").arg(ev.returnCode));
        ui->textBrowser_2->append(QString::fromUtf8("成功向单元%1发送拍照指令！").arg(str.c_str()));
    }
    else
        ui->textBrowser_2->append(QString::fromUtf8("操作失败，单元%1不存在！").arg(str.c_str()));
}
