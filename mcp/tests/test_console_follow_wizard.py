...
def test_two_vehicle_leader_selection_stays_inside_task_level_flow():
    state = ConsoleFollowWizardState()
...
def test_confirmation_builds_one_exact_follow_payload():
    ...
    assert result.tool_name == "createFollowFormation"
...
def test_target_input_calls_only_follow_target_tool(text):
    ...
    assert result.tool_name == "moveFollowFormation"
    assert result.tool_args == {"target": {"x": 5.0, "y": 6.0}}
