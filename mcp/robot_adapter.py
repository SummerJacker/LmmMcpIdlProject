...
class RobotAdapter:
    ...
    async def create_air_ground_formation(
        ...
        air_leader = await self._resolve_to_unit_id(air_leader_id)
        if air_leader is None:
            return make_tool_response(
                success=False,
                message=_message_unit_not_bound(air_leader_id),
                error_code="UNIT_NOT_FOUND",
            )
        if not air_leader.upper().startswith("A"):
            return make_tool_response(
                success=False,
                message="Validation failed: air leader must be an A* (UAV) unit",
                error_code="SAFETY_REJECTED",
            )
        ok_alt, altitude, error_alt = _finite_float(air_altitude_m, "altitude_m")
        if not ok_alt or altitude < 0.0:
            return make_tool_response(
                success=False,
                message=error_alt or "Validation failed: altitude_m must be non-negative",
                error_code="SAFETY_REJECTED",
            )
        try:
            raw_followers = json.loads(followers_json)
            ...
            return make_tool_response(success=False, message="Validation failed: followers must be a JSON array")
        if not isinstance(raw_followers, list) or not raw_followers:
            return make_tool_response(success=False, message="Validation failed: at least one ground follower is required")
        from safety.validator import validate_follow_distance
        ...
        for item in raw_followers:
            ...
            if not follower.upper().startswith("G"):
                return make_tool_response(
                    success=False,
                    message=f"Validation failed: ground follower {follower} must be a G* (UGV) unit",
                    error_code="SAFETY_REJECTED",
                )
            ok, distance, error = _finite_float(item.get("distance_m"), "distance_m")
            if not ok:
                return make_tool_response(
                    success=False, message=error, error_code="SAFETY_REJECTED"
                )
            passed, error_code, validation_message = validate_follow_distance(distance)
            if not passed:
                return make_tool_response(
                    success=False, message=validation_message, error_code=error_code
                )
            ok_a, angle, error_a = _finite_float(item.get("angle_deg", 0.0), "angle_deg")
            if not ok_a or angle < -180.0 or angle > 180.0:
                return make_tool_response(
                    success=False,
                    message=error_a or "Validation failed: angle_deg must be within [-180, 180]",
                    error_code="SAFETY_REJECTED",
                )
            if follower in seen:
                return make_tool_response(success=False, message=f"Validation failed: duplicate unit {follower}")
            seen.add(follower)
            followers.append({"unit_id": follower, "distance_m": distance, "angle_deg": angle})
        return await self._post_qt(
            path=QT_AIR_GROUND_FORMATION_PATH,
            json_body={"air_leader": {"unit_id": air_leader, "altitude_m": altitude},
                       "ground_followers": followers},
            robot_id_for_lock=None,
            op_name="create_air_ground_formation",
        )
