 ...
 */
public final class TheRS { 

    public static int _szFunc(xerox.ilu.IluCall _call, TheR[] _v)
                throws org.omg.CORBA.SystemException {
        int _sz = _call.szSequence(_v.length, 0);
        for (int _i = 0; _i < _v.length; _i++) {
            _sz += TheRHelper._szFunc(_call, _v[_i]);
        }
        _call.endSequence();
        return _sz;
    } //_szFunc

    public static void _outFunc(xerox.ilu.IluCall _call, TheR[] _v)
                throws org.omg.CORBA.SystemException {
        _call.outSequence(_v.length, 0);
        for (int _i = 0; _i < _v.length; _i++) {
            TheRHelper._outFunc(_call, _v[_i]);
        }
        _call.endSequence();
    } //_outFunc
    ...
    public static TheR[] _inFunc(xerox.ilu.IluCall _call)
        ...
        TheR[] _v = new TheR[_ln];
        for (int _i = 0; _i < _ln; _i++) {
            _v[_i] = TheRHelper._inFunc(_call);
        }
        _call.endSequence();
        return _v;
    } //_inFunc
