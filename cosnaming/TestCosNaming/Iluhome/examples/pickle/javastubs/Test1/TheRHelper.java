// javastubs/Test1/TheRHelper.java
// Stubs for "Test1"
//
// This file was automatically generated with KISORB (version 2.0beta1) tools
// at Fri Aug 22 17:23:39 2025 by `沈阳市软件定义智能协同重点实验室'
// running "D:/Shaun-projects/newstar/Iluhome/bin/java-stubber.exe" of Mon Aug 18 11:47:37 2025
// on "D:/Shaun-projects/newstar/Iluhome/examples/pickle//Test1.isl" of Fri Aug 01 14:00:48 2025,
// and "D:/Shaun-projects/newstar/Iluhome/examples/pickle//ilu.isl" of Fri Aug 01 09:16:44 2025//
// KISORB is Copyright 2006-2056 成都凯斯人工智能研究院, All Rights Reserved.
// KISORB information: Dr. Wang xcwang89@aliyun.com .



package Test1;


/** 
 * An ILU helper class.
 */
public class TheRHelper implements xerox.ilu.IluIOFunctions  {
    private final static java.lang.String _id = 
        "ilut:aRm1NNwKmLTCta84rO4Q+MwTPU7";
    private static xerox.ilu.IluTypeCode _tc = null;
    static {
        xerox.ilu.IluPreLoad.checkStubConsistency13("v2 (2.0beta1)");
        A1Helper.id();
        CSSHelper.id();
        IHelper.id();
        _tc = xerox.ilu.IluTypeCode.newTypeCode(id(), new TheRHelper(), org.omg.CORBA.TCKind.tk_struct);
    }

    public final static java.lang.String id() {
        return _id;
    } //id 

    public final static xerox.ilu.IluTypeCode type() {
        return _tc;
    } //type 

    public static void insert(org.omg.CORBA.Any _any, TheR _x) throws org.omg.CORBA.SystemException {
        xerox.ilu.IluAny _ia = (xerox.ilu.IluAny) _any;
        _ia.assign(_tc, _x);
    } //insert 

    public static TheR extract(org.omg.CORBA.Any _any) throws org.omg.CORBA.SystemException {
        xerox.ilu.IluAny _ia = (xerox.ilu.IluAny) _any;
        java.lang.Object _ob = _ia.cachedValue();
        if (_ob == null) {_ob = _ia.value(_tc);}
        return ((TheR)_ob);
    } //extract 

    public static xerox.ilu.IluPickle to_pickle(TheR _x) throws org.omg.CORBA.SystemException {
        int _sz = 0;
        xerox.ilu.IluCall _call = null;
        xerox.ilu.IluPickle _pickle = new xerox.ilu.IluPickle();
        try {
            _call = _pickle.startToPickle();
            _sz = TheRHelper._szFunc(_call, _x);
            _pickle.midToPickle(_call, _sz, id());
            TheRHelper._outFunc(_call, _x);
        } finally {
            _pickle.endToPickle(_call);
        }
        return _pickle;
    } //to_pickle 

    public static TheR from_pickle(xerox.ilu.IluPickle _pickle) throws org.omg.CORBA.SystemException {
        TheR _x = null;
        xerox.ilu.IluCall _call = null;
        try {
            _call = _pickle.startFromPickle(id());
            _x = TheRHelper._inFunc(_call);
        } finally {
            _pickle.endFromPickle(_call);
        }
        return _x;
    } //from_pickle 

    /** Not public; implements xerox.ilu.IluIOFunctions */
    public int szFunc(xerox.ilu.IluCall _call, java.lang.Object _x, xerox.ilu.IluTypeCode _tc) throws org.omg.CORBA.SystemException {
        TheR _xx = ((TheR)_x);
        return TheRHelper._szFunc(_call, _xx);
    } //szFunc 

    /** Not public; implements xerox.ilu.IluIOFunctions */
    public void outFunc(xerox.ilu.IluCall _call, java.lang.Object _x, xerox.ilu.IluTypeCode _tc) throws org.omg.CORBA.SystemException {
        TheR _xx = ((TheR)_x);
        TheRHelper._outFunc(_call, _xx);
    } //outFunc 

    /** Not public; implements xerox.ilu.IluIOFunctions */
    public java.lang.Object inFunc(xerox.ilu.IluCall _call, xerox.ilu.IluTypeCode _tc) throws org.omg.CORBA.SystemException {
        return TheRHelper._inFunc(_call);
    } //inFunc 

    public boolean isAFunc(java.lang.Object _x, xerox.ilu.IluTypeCode _tc) {
        if (_x == null) return false;
        return (_x instanceof TheR);
    } //isAFunc 

    static {
        Test1._allJavaStubs.load();
    }
    static {
        xerox.ilu.IluTypeRep __t = null;
        __t = xerox.ilu.IluTypeRep.registerRecordType(
            "TheR", //name
            "Test1", //islIfName
            null, //islIfBrand
            "ilut:aRm1NNwKmLTCta84rO4Q+MwTPU7", //uid
            3 //cnt
            );
        __t.registerRecordField(0, "a", "ilut:h3Dnvis94UwnmFURfYsCp700jVy");
        __t.registerRecordField(1, "css", "ilut:f-6N-TJeDtWrjsoVoolxLZVOEJy");
        __t.registerRecordField(2, "i", "ilut:ggrbg-q7UoqiAlNTWvqkwfZ4rlY");
        __t.finish();
        __t = null;
    }

    //internal to ILU only
    public static int _szFunc(xerox.ilu.IluCall _call, TheR _r)
                throws org.omg.CORBA.SystemException {
        int _sz = _call.szRecord();
        _sz = _sz + TheA1.szFunc(_call, _r.a);
        _sz = _sz + CSS._szFunc(_call, _r.css);
        _sz = _sz + _call.szInt32(_r.i);
        _call.endRecord();
        return _sz;
    } //_szFunc

    //internal to ILU only
    public static void _outFunc(xerox.ilu.IluCall _call, TheR _r)
                throws org.omg.CORBA.SystemException {
        _call.outRecord();
        TheA1.outFunc(_call, _r.a);
        CSS._outFunc(_call, _r.css);
        _call.outInt32(_r.i);
        _call.endRecord();
    } //_outFunc

    //internal to ILU only
    public static TheR _inFunc(xerox.ilu.IluCall _call)
                throws org.omg.CORBA.SystemException {
        TheR _r = TheR.alloc_TheR();
        _call.inRecord();
        _r.a = TheA1.inFunc(_call);
        _r.css = CSS._inFunc(_call);
        _r.i = _call.inInt32();
        _call.endRecord();
        _r._touch();
        return _r;
    } //_inFunc

}//TheRHelper

