...
*/
/* $Id: NamingImpl.c,v 2.2 2025/08/01 00:17:19 cvswxc Exp $ */


/*
 Revision 2.1  2020/06/12 03:22:26  cvswky
 #658 linux下gcc 未定义TRUE FALSE，替换为ilu_TRUE,ilu_FALSE

 Revision 2.0  2014/04/15 08:53:12  cvsczh
 no message

 Revision 1.3  2013/05/24 09:22:10  cvswxc
 强化判断->local的值一定与TRUE/FALSE比较，加强程序的健壮性。 #327

 Revision 1.2  2011/04/06 04:10:23  cvswxc
 将文件路径设为相对路经, 或不hard-coded进程序, 见 #23
	     ...
	     ILU_C_ENVIRONMENT *_status)
{
  ...
  if ((size = CosNaming_Name_Length(n)) == 0) {
	...
	ilu_string sbh;
	nv->local = ilu_FALSE;
	sbh = CORBA_ORB_object_to_string(ILU_C_ORB, obj, _status);
	if (!ILU_C_SUCCESSFUL(_status) || sbh == ILU_NIL) {
	  ILU_C_RAISE_SYSTEM(_status,INTERNAL,0,NO);
				       ...
				       ILU_C_ENVIRONMENT *_status)
{
  ...
  time_t current = time(NULL); ///Shaun ADD
  ...
  if (size > 1) {
    ...
    if (ilu_TRUE == nv->local) {
      ...
      debugprint("ILUCosNaming:  resolve:  %s For \"%s\", found remote context \"%s\"\n",
	      ctime(&current), nv->key.id, nv->value.sbh); //Shaun MDF
      return ILU_C_SBHToObject(nv->value.sbh, CosNaming_NamingContext__MSType, _status);
    } else {
      debugprint("ILUCosNaming:  resolve:  %s For \"%s\", found remote object \"%s\"\n",
	      ctime(&current), nv->key.id, nv->value.sbh); //Shaun MDF
      if (strncmp(nv->value.sbh, "IOR:", 4) == 0)
	return (ilu_CORBA_Object) CORBA_ORB_string_to_object(ILU_C_ORB, nv->value.sbh, _status);
      else
...
InitializeRoot (char *filename, char *root_sid, ILU_C_Server server)
{
  ...
  if (ftell(fp) > 0) {
	...
	if (!ilu_hash_AddToTable(ht, nv->key.id, nv)) {
	  ...
	  if (ilu_FALSE == nv->local) ilu_free(sbh);
	  ilu_free(nv);
	  return ILU_NIL;
	};
      }
...
int main (int ac, char **av)
{
  ...
  if (BackingFilename == ILU_NIL)
    {
      sprintf (hostname, "ILUCosNaming-%s.txt", sid);
  ...
  fprintf(stdout, "Root Naming SBH: %s\n", ILU_C_SBHOfObject(root));
  fflush(stdout);
